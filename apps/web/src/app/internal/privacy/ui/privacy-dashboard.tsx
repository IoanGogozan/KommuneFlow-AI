...
export function PrivacyDashboard() {
  ...
  async function loadPrivacyContext() {
    const loadedStatus = (await callPrivacy("/privacy/status")) as
      | PrivacyStatus
      | null;
    setStatus(loadedStatus);

    const loadedPolicy = (await callPrivacy("/privacy/retention-policy")) as
      | RetentionPolicy
      | null;
  ...
  async function updatePolicy(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      closedCaseRetentionDays: Number(
        formData.get("closedCaseRetentionDays"),
      ),
      deletedDocumentRetentionDays: Number(
        formData.get("deletedDocumentRetentionDays"),
      ),
      auditEventRetentionDays: Number(
        formData.get("auditEventRetentionDays"),
      ),
      ...
      <div className="mx-auto max-w-6xl px-5 py-6">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-300 pb-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <NavLink href="/internal/cases" label={t.nav.cases} />
            <NavLink href="/internal/analytics" label={t.nav.analytics} />
            <NavLink href="/internal/operations" label={t.nav.operations} />
          </div>
        </header>
