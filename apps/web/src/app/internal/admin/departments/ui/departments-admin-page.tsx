...
export function DepartmentsAdminPage() {
  ...
  useEffect(() => {
    async function loadDepartments() {
      ...
      try {
        ...
        if (!response.ok) {
          setError(t.admin.departments.loadError);
          return;
        }
  ...
  }, [canViewDepartments, currentUser, router, sessionLoading, t.admin.departments.loadError]);
    ...
    return (
      ...
      >
        <p className="mt-6 text-sm text-slate-600">
          {sessionError ? t.admin.departments.loadError : t.cases.loading}
        </p>
      </InternalShell>
    );
  }
      ...
      <section className="mt-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-950">
              {t.admin.departments.sectionTitle}
            </h2>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              {t.admin.departments.description} {currentUser.tenant.name}.
            </p>
          </div>
          <span className="rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">
            {departments.length} {t.admin.departments.count}
          </span>
        </div>
        ...
        <div className="mt-5 overflow-hidden rounded-md border border-slate-200">
          <div className="hidden grid-cols-[1.2fr_1fr_1fr_0.6fr] gap-3 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-normal text-slate-500 md:grid">
            <span>{t.admin.departments.columns.name}</span>
            <span>{t.admin.departments.columns.slug}</span>
            <span>{t.admin.departments.columns.municipality}</span>
            <span className="text-right">{t.admin.departments.columns.cases}</span>
          </div>
